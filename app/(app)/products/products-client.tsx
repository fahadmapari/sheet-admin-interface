  ...
  const filteredProducts = useMemo(() => {
    ...
    return all.filter((p) => {
      for (const { key, productKey } of MULTI_SELECT_FILTERS) {
        if (!matchesMultiFilter(p[productKey], filters[key])) return false;
      }
      for (const { key, productKey } of TRI_STATE_FILTERS) {
        if (!matchesTriStateFilter(p[productKey], filters[key])) return false;
      }
      for (const { key, productKey } of PRESENCE_FILTERS) {
        if (!matchesPresenceFilter(p[productKey], filters[key])) return false;
      }
      return true;
    });
  }, [products, filters]);
